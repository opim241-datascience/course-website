...
- [Click here to go the the Assignment in Canvas](https://georgetown.instructure.com/courses/126656/assignments/565092)
- Click on the link inside the assignment to go to the acceptance page. You should see something like the following screen, but the name of the repository that will be created is `assignment-02-YOURGITHUBUSERNAME`:

Note: the screen shots are for reference, the repository names will be different.

```{r echo=FALSE, eval=TRUE}
knitr::include_graphics("img/gh1.png")
```

- Click on the green _Accept the assignment_ button and you will see a confirmation page:

```{r echo=FALSE, eval=TRUE}
knitr::include_graphics("img/gh2.png")
```

- Click on the [Course's GitHub Organization Page](https://github.com/opim241-datascience) and you will see your private repository there. Rememeber you need to be logged into GitHub.

- Grab the URL of **YOUR** repo, and clone it in RStudio.
Refer to [Lab 1](https://opim241.netlify.app/lab-instructions/lab-01/lab-01-hello-r.html) if you would like to see step-by-step instructions for cloning a repo into an RStudio project.

- Open the R Markdown document `hw-02.Rmd` and Knit it.
...
We'll use the **tidyverse** package for much of the data wrangling and visualization and the data lives in the **dsbox** package. You may need to install the **dsbox** package.
